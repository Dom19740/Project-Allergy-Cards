...
const AllergyCard: React.FC<AllergyCardProps> = ({ languageCode, selectedAllergens }) => {
  ...
  // Ref + state for sizing the no-entry overlay to match one grid cell
  ...
  useEffect(() => {
    const update = () => {
      ...
      const cols = allergensWithImages.length >= 5 ? 3 : allergensWithImages.length >= 2 ? 2 : 1;
      // Ensure at least 1
      const cell = Math.max(0, Math.floor(gridWidth / Math.max(1, cols)));
      setOverlaySize({ w: cell, h: cell });
    };
    ...
    >
      ...
      {allergensWithImages.length > 0 && (
        <div className="w-full flex items-center justify-center">
          <div ref={gridRef} className={`relative ${imageGridClasses} ${rowClasses} gap-0`}> 
