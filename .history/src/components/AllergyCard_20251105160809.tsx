...
const AllergyCard: React.FC<AllergyCardProps> = ({ languageCode, selectedAllergens }) => {
  ...
  if (allergensWithImages.length === 1) {
    ...
    // If exactly 2 images, center the grid content so the two items sit centrally under the overlay
    if (allergensWithImages.length === 2) gridAlignClasses = "place-content-center";
    ...
    >
      ...
      {allergensWithImages.length > 0 && (
        <div className="w-full flex items-center justify-center">
          {/* Fixed-sized container for overlay (fills available width up to 90vmin) */}
          <div className="relative w-full max-w-[90vmin] aspect-square">
            {/* Grid is positioned absolutely to fill the fixed container; gap-0 keeps rows tight */}
            <div className={`${imageGridClasses} ${rowClasses} ${gridAlignClasses} gap-0 absolute inset-0 w-full h-full`}>
            ...
            {/* Overlay sits in its own centered container so its size is independent of grid content */}
            <div className="absolute inset-0 flex items-center justify-center p-2 pointer-events-none z-10">
              <img src="/noentry.png" alt="No entry" className="w-full h-full object-contain" />
            </div>
          </div>
        </div>
      )}
      ...
      <Button 
        onClick={handleShare} 
        disabled={isSharing}
        className="mt-4 bg-green-600 text-white hover:bg-green-700 w-8 h-8 flex items-center justify-center relative z-20"
      >
