...
const AllergyCard: React.FC<AllergyCardProps> = ({ languageCode, selectedAllergens }) => {
  ...
  return (
    <div 
      ref={cardRef} 
      className="flex flex-col items-center justify-between w-full bg-white text-foreground p-3 sm:p-6 text-center relative overflow-hidden pb-16 h-[100dvh]"
    >
      ...
      {allergensWithImages.length > 0 && (
        <div className="relative w-full max-w-[200px] sm:max-w-[240px] aspect-square mx-auto my-2">
          <div className={`absolute inset-0 ${imageGridClasses} gap-0.5 items-center justify-center`}>
      ...
      {text.scanMe && (
        <p className="text-lg sm:text-xl md:text-2xl font-medium mt-4 mb-3">{text.scanMe}</p>
      )}

      <div className="flex items-center justify-center mb-4">
        <QRCode
          value={windowLocation.href}
          size={200}
          level="H"
          fgColor="#000"
          bgColor="transparent"
          className="p-2 sm:p-4 bg-white rounded-lg"
        />
      </div>
      ...
      <Button 
        onClick={handleShare} 
        disabled={isSharing}
        className="mt-2 bg-green-600 text-white hover:bg-green-700 w-12 h-12 flex items-center justify-center"
      >
